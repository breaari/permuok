...
export default function ConversationGroupCard({ group, onOpen }) {
  const count = Number(group?.conversation_count || 0);

  const unread = Number(group?.unread_count || 0);

  const latestConversation = group?.last_conversation || null;

  const preview = latestConversation
    ? buildConversationPreview(latestConversation)
    : "";
  return (
    <article
      ...
      <button type="button" onClick={onOpen} className="w-full text-left">
        <div className="flex items-start gap-3 sm:gap-4">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700">
            <Icon name="messagesSquare" size={21} />
          </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-slate-600">
                  {getOpportunityLabel(group?.opportunity_type)}
                </span>

                <h3 className="mt-3 line-clamp-2 text-base font-black text-slate-900">
                  {group?.subject || "Publicación"}
                </h3>
              </div>

              <p className="text-xs font-bold text-slate-400">
                {formatConversationDate(group?.last_activity_at)}
              </p>
            </div>
