...
import { getMultilateralCompatibilities } from "../api/compatibilities.api";
...
function resolveDifferenceLabel(item) {
  const difference = Number(item?.own_cash_difference || 0);
  const currency = item?.own_comparison_currency || "USD";
  ...
  if (item?.own_direction === "a_favor") {
    return {
      label: `Aportás ${formatMoney(difference, currency)}`,
      description:
  ...
  return {
    ...
    description: "Diferencia estimada de la operación.",
    tone: "slate",
  };
}
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <h2 className="mt-1 truncate text-lg font-black tracking-tight text-slate-900">
                {item?.participants_count || 0} inmobiliarias conectadas
              </h2>
            </div>
          </div>
      ...
      <div className="p-5">
        <div className="grid gap-3 md:grid-cols-2">
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div className="mt-1 font-bold text-slate-900">
              {item?.offered_property_title || "Propiedad publicada"}
            </div>
          ...
          <div className="rounded-xl border border-violet-100 bg-violet-50/60 p-4">
            ...
            <div className="mt-1 font-bold text-slate-900">
              {item?.target_property_title || "Propiedad objetivo"}
            </div>
            ...
            {(item?.target_property_city || item?.target_property_zone) && (
              <div className="mt-1 truncate text-xs text-slate-400">
                {[item?.target_property_zone, item?.target_property_city]
                  .filter(Boolean)
                  .join(", ")}
              </div>
            )}
        ...
        <div
          className={`mt-4 rounded-xl border p-3 ${
            differenceStyles[difference.tone] || differenceStyles.slate
          }`}
        >
...
export default function MultilateralCompatibilities() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const data = await getMultilateralCompatibilities({
        page,
        limit: 12,
        view,
      });
      ...
      console.error("Error cargando oportunidades multilaterales:", err);
        ...
        <div className="mt-1 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-500">
              Oportunidades que conectan tres o más inmobiliarias para completar
              una cadena de permutas compatible.
            </p>
          </div>
        ...
        <div className="mb-7 grid grid-cols-2 gap-3 lg:grid-cols-3">
          <div className="rounded-2xl border border-violet-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-xs text-slate-500">
              Cadenas completas disponibles actualmente.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-xs text-slate-500">
              Promedio de las oportunidades visibles.
            </p>
          </div>
          ...
          <div className="col-span-2 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:col-span-1">
            <div className="text-3xl font-black text-slate-900">
              {items.reduce(
                (acc, item) => acc + Number(item?.participants_count || 0),
                0,
              )}
            </div>
            <div className="mt-1 text-sm font-bold text-slate-800">
              Participaciones conectadas
            </div>
            <p className="mt-1 text-xs text-slate-500">
              Inmobiliarias vinculadas en estas cadenas.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {items.map((item) => (
            <MultilateralCard
              ...
              item={item}
              onOpen={(operation) =>
                navigate(`/compatibilities/multilateral/${operation.id}`)
              }
            />
          ))}
        </div>
