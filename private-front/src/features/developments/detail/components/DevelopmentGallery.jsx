...
import { formatStage } from "../developmentDetail.helpers";
    ...
    <div className="space-y-4 lg:col-span-8">
      ...
      >
        {mainImage ? (
          <img
            src={mainImage.url}
            alt={development?.title || "Desarrollo"}
        ...
        {development?.development_stage ? (
          <div className="absolute right-4 top-4 rounded-full border border-white/20 bg-white/70 px-4 py-2 backdrop-blur-md">
            <div className="flex items-center gap-2">
              <Icon name="building2" size={16} className="text-emerald-700" />
              <span className="text-xs font-bold uppercase tracking-wider text-slate-900">
                {formatStage(development?.development_stage)}
              </span>
            </div>
          </div>
        ) : null}
