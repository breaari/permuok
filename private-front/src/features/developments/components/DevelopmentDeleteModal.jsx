...
export default function DevelopmentDeleteModal({
  open,
  busy = false,
  developmentTitle = "",
  developmentImageUrl = "",
  ...
  const imageUrl = developmentImageUrl
    ? developmentImageUrl.startsWith("http")
      ? developmentImageUrl
      : `${getApiBaseUrl()}${developmentImageUrl}`
    : "";
      ...
      >
        ...
        <div className="p-6 sm:p-8">
            ...
            <div className="mt-3 flex items-center gap-3">
              <div className="h-16 w-16 overflow-hidden rounded-lg border border-slate-200 bg-white shrink-0">
                {imageUrl ? (
                  <img
                    src={imageUrl}
                    alt={developmentTitle || "Desarrollo"}
              ...
              <div className="min-w-0">
                <p className="text-base sm:text-lg font-bold text-slate-900 break-words">
                  {developmentTitle || "Desarrollo sin título"}
                </p>
              </div>
            </div>
          </div>
          ...
          <p className="text-sm leading-relaxed text-slate-600">
            El desarrollo dejará de aparecer en tus listados, búsquedas,
            matches y experiencia activa. El historial relacionado podrá
            conservarse como registro, pero el desarrollo dejará de estar
            disponible.
          </p>
        </div>
        ...
        <div className="border-t border-slate-100 bg-slate-50 px-6 py-4 sm:px-8">
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            >
              {busy ? "Eliminando..." : "Eliminar desarrollo"}
            </button>
          </div>
        </div>
